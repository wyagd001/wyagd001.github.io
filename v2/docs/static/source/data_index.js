indexData = [
  ["-","Variables.htm#AddSub",4],
  ["- (sign)","Variables.htm#unary",4],
  ["--","Variables.htm#IncDec",4],
  ["-=","Variables.htm#AssignOp",4],
  [",","Variables.htm#comma",4],
  [":","Variables.htm#ternary",4],
  [":=","Variables.htm#AssignOp",4],
  ["!","Variables.htm#unary",4],
  ["!=","Variables.htm#equal",4],
  ["!==","Variables.htm#equal",4],
  ["?","Variables.htm#ternary",4],
  ["??","Variables.htm#or-maybe",4],
  [".","Variables.htm#concat",4],
  [".=","Variables.htm#AssignOp",4],
  ["[]","Variables.htm#square-brackets"],
  ["{}","Variables.htm#curly-braces"],
  ["*","Variables.htm#MulDiv",4],
  ["**","Variables.htm#pow",4],
  ["*=","Variables.htm#AssignOp",4],
  ["/","Variables.htm#MulDiv",4],
  ["//","Variables.htm#MulDiv",4],
  ["//=","Variables.htm#AssignOp",4],
  ["/=","Variables.htm#AssignOp",4],
  ["&","Variables.htm#ref",4],
  ["& (bitwise-and)","Variables.htm#bitwise",4],
  ["&&","Variables.htm#and",4],
  ["&=","Variables.htm#AssignOp",4],
  ["#ClipboardTimeout","commands/_ClipboardTimeout.htm",0,"S"],
  ["#DllLoad","commands/_DllLoad.htm",0,"S"],
  ["#ErrorStdOut","commands/_ErrorStdOut.htm",0,"S"],
  ["#HotIf","commands/_HotIf.htm",0,"E"],
  ["#HotIfTimeout","commands/_HotIfTimeout.htm",0,"S"],
  ["#Hotstring","commands/_Hotstring.htm",0,"S"],
  ["#Include","commands/_Include.htm",0,"S"],
  ["#IncludeAgain","commands/_Include.htm",0,"S"],
  ["#InputLevel","commands/_InputLevel.htm",0,"S"],
  ["#MaxThreads","commands/_MaxThreads.htm",0,"S"],
  ["#MaxThreadsBuffer","commands/_MaxThreadsBuffer.htm",0,"S"],
  ["#MaxThreadsPerHotkey","commands/_MaxThreadsPerHotkey.htm",0,"S"],
  ["#NoTrayIcon","commands/_NoTrayIcon.htm",0,""],
  ["#Requires","commands/_Requires.htm",0,"S"],
  ["#SingleInstance","commands/_SingleInstance.htm",0,"S"],
  ["#SuspendExempt","commands/_SuspendExempt.htm",0,"S"],
  ["#UseHook","commands/_UseHook.htm",0,"S"],
  ["#Warn","commands/_Warn.htm",0,"SS"],
  ["#WinActivateForce","commands/_WinActivateForce.htm",0,""],
  ["%Expr%","Variables.htm#deref"],
  [";@Ahk2Exe-","misc/Ahk2ExeDirectives.htm"],
  ["@Ahk2Exe-","misc/Ahk2ExeDirectives.htm"],
  ["^","Variables.htm#bitwise",4],
  ["^=","Variables.htm#AssignOp",4],
  ["+","Variables.htm#AddSub",4],
  ["++","Variables.htm#IncDec",4],
  ["+=","Variables.htm#AssignOp",4],
  ["<","Variables.htm#compare",4],
  ["<<","Variables.htm#bitshift",4],
  ["<<=","Variables.htm#AssignOp",4],
  ["<=","Variables.htm#compare",4],
  ["=","Variables.htm#equal",4],
  ["==","Variables.htm#equal",4],
  ["=>","Variables.htm#fat-arrow",4],
  [">","Variables.htm#compare",4],
  [">=","Variables.htm#compare",4],
  [">>","Variables.htm#bitshift",4],
  [">>>","Variables.htm#bitshift",4],
  [">>=","Variables.htm#AssignOp",4],
  [">>>=","Variables.htm#AssignOp",4],
  ["|","Variables.htm#bitwise",4],
  ["|=","Variables.htm#AssignOp",4],
  ["||","Variables.htm#or",4],
  ["~","Variables.htm#unary",4],
  ["~=","Variables.htm#regex",4],
  ["__Call meta-function","Objects.htm#Meta_Functions"],
  ["__Delete meta-function","Objects.htm#Custom_NewDelete"],
  ["__Get meta-function","Objects.htm#Meta_Functions"],
  ["__Enum method","Objects.htm#__Enum"],
  ["__Enum method (Array)","objects/Array.htm#__Enum",7],
  ["__Enum method (Gui)","objects/Gui.htm#__Enum",7],
  ["__Enum method (Map)","objects/Map.htm#__Enum",7],
  ["__Init method","Objects.htm#Custom_Classes_var"],
  ["__Item property","Objects.htm#__Item"],
  ["__Item property (Array)","objects/Array.htm#__Item",7],
  ["__Item property (Gui)","objects/Gui.htm#__Item",7],
  ["__Item property (Map)","objects/Map.htm#__Item",7],
  ["__New meta-function","Objects.htm#Custom_NewDelete"],
  ["__New method (Array)","objects/Array.htm#__New",7],
  ["__New method (Buffer)","objects/Buffer.htm#__New",7],
  ["__New method (Gui)","objects/Gui.htm#__New",7],
  ["__New method (Map)","objects/Map.htm#__New",7],
  ["__Set meta-function","Objects.htm#Meta_Functions"],
  ["A_AhkPath","Variables.htm#AhkPath",1],
  ["A_AhkVersion","Variables.htm#AhkVersion",1],
  ["A_AllowMainWindow","Variables.htm#AllowMainWindow",1],
  ["A_AppData","Variables.htm#AppData",1],
  ["A_AppDataCommon","Variables.htm#AppDataCommon",1],
  ["A_Args","Variables.htm#Args",1],
  ["A_BasePath (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#BasePath",99],
  ["A_Clipboard","misc/A_Clipboard.htm",1],
  ["A_ComputerName","Variables.htm#ComputerName",1],
  ["A_ComSpec","Variables.htm#ComSpec",1],
  ["A_ControlDelay","Variables.htm#ControlDelay",1],
  ["A_CoordModeCaret","Variables.htm#CoordMode",1],
  ["A_CoordModeMenu","Variables.htm#CoordMode",1],
  ["A_CoordModeMouse","Variables.htm#CoordMode",1],
  ["A_CoordModePixel","Variables.htm#CoordMode",1],
  ["A_CoordModeToolTip","Variables.htm#CoordMode",1],
  ["A_Cursor","Variables.htm#Cursor",1],
  ["A_DD","Variables.htm#DD",1],
  ["A_DDD","Variables.htm#DDDD",1],
  ["A_DDDD","Variables.htm#DDDD",1],
  ["A_DefaultMouseSpeed","Variables.htm#DefaultMouseSpeed",1],
  ["A_Desktop","Variables.htm#Desktop",1],
  ["A_DesktopCommon","Variables.htm#DesktopCommon",1],
  ["A_DetectHiddenText","Variables.htm#DetectHiddenText",1],
  ["A_DetectHiddenWindows","Variables.htm#DetectHiddenWindows",1],
  ["A_EndChar","Variables.htm#EndChar",1],
  ["A_EventInfo","Variables.htm#EventInfo",1],
  ["A_FileEncoding","Variables.htm#FileEncoding",1],
  ["A_HotkeyInterval","misc/A_MaxHotkeysPerInterval.htm",1],
  ["A_HotkeyModifierTimeout","misc/A_HotkeyModifierTimeout.htm",1],
  ["A_Hour","Variables.htm#Hour",1],
  ["A_IconFile","Variables.htm#IconFile",1],
  ["A_IconHidden","Variables.htm#IconHidden",1],
  ["A_IconNumber","Variables.htm#IconNumber",1],
  ["A_IconTip","Variables.htm#IconTip",1],
  ["A_Index","commands/Loop.htm",1],
  ["A_InitialWorkingDir","Variables.htm#InitialWorkingDir",1],
  ["A_Is64bitOS","Variables.htm#Is64bitOS",1],
  ["A_IsAdmin","Variables.htm#IsAdmin",1],
  ["A_IsCompiled","Variables.htm#IsCompiled",1],
  ["A_IsCritical","Variables.htm#IsCritical",1],
  ["A_IsPaused","Variables.htm#IsPaused",1],
  ["A_IsSuspended","Variables.htm#IsSuspended",1],
  ["A_KeyDelay","Variables.htm#KeyDelay",1],
  ["A_KeyDelayPlay","Variables.htm#KeyDelayPlay",1],
  ["A_KeyDuration","Variables.htm#KeyDelay",1],
  ["A_KeyDurationPlay","Variables.htm#KeyDelayPlay",1],
  ["A_Language","Variables.htm#Language",1],
  ["A_Language Values","misc/Languages.htm"],
  ["A_LastError","Variables.htm#LastError",1],
  ["A_LineFile","Variables.htm#LineFile",1],
  ["A_LineNumber","Variables.htm#LineNumber",1],
  ["A_ListLines","Variables.htm#ListLines",1],
  ["A_LoopField","commands/LoopParse.htm#LoopField",1],
  ["A_LoopFileAttrib","commands/LoopFiles.htm#LoopFileAttrib",1],
  ["A_LoopFileDir","commands/LoopFiles.htm#LoopFileDir",1],
  ["A_LoopFileExt","commands/LoopFiles.htm#LoopFileExt",1],
  ["A_LoopFileFullPath","commands/LoopFiles.htm#LoopFileFullPath",1],
  ["A_LoopFileName","commands/LoopFiles.htm#LoopFileName",1],
  ["A_LoopFilePath","commands/LoopFiles.htm#LoopFilePath",1],
  ["A_LoopFileShortName","commands/LoopFiles.htm#LoopFileShortName",1],
  ["A_LoopFileShortPath","commands/LoopFiles.htm#LoopFileShortPath",1],
  ["A_LoopFileSize","commands/LoopFiles.htm#LoopFileSize",1],
  ["A_LoopFileSizeKB","commands/LoopFiles.htm#LoopFileSizeKB",1],
  ["A_LoopFileSizeMB","commands/LoopFiles.htm#LoopFileSizeMB",1],
  ["A_LoopFileTimeAccessed","commands/LoopFiles.htm#LoopFileTimeAccessed",1],
  ["A_LoopFileTimeCreated","commands/LoopFiles.htm#LoopFileTimeCreated",1],
  ["A_LoopFileTimeModified","commands/LoopFiles.htm#LoopFileTimeModified",1],
  ["A_LoopReadLine","commands/LoopRead.htm#LoopReadLine",1],
  ["A_LoopRegKey","commands/LoopReg.htm#vars",1],
  ["A_LoopRegName","commands/LoopReg.htm#vars",1],
  ["A_LoopRegTimeModified","commands/LoopReg.htm#vars",1],
  ["A_LoopRegType","commands/LoopReg.htm#vars",1],
  ["A_MaxHotkeysPerInterval","misc/A_MaxHotkeysPerInterval.htm",1],
  ["A_MenuMaskKey","misc/A_MenuMaskKey.htm",1],
  ["A_MDay","Variables.htm#DD",1],
  ["A_Min","Variables.htm#Min",1],
  ["A_MM","Variables.htm#MM",1],
  ["A_MMM","Variables.htm#MMM",1],
  ["A_MMMM","Variables.htm#MMMM",1],
  ["A_Mon","Variables.htm#MM",1],
  ["A_MouseDelay","Variables.htm#MouseDelay",1],
  ["A_MouseDelayPlay","Variables.htm#MouseDelay",1],
  ["A_MSec","Variables.htm#MSec",1],
  ["A_MyDocuments","Variables.htm#MyDocuments",1],
  ["A_Now","Variables.htm#Now",1],
  ["A_NowUTC","Variables.htm#NowUTC",1],
  ["A_OSVersion","Variables.htm#OSVersion",1],
  ["A_PriorHotkey","Variables.htm#PriorHotkey",1],
  ["A_PriorKey","Variables.htm#PriorKey",1],
  ["A_PriorLine (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#PriorLine",99],
  ["A_ProgramFiles","Variables.htm#ProgramFiles",1],
  ["A_Programs","Variables.htm#Programs",1],
  ["A_ProgramsCommon","Variables.htm#ProgramsCommon",1],
  ["A_PtrSize","Variables.htm#PtrSize",1],
  ["A_RegView","Variables.htm#RegView",1],
  ["A_ScreenDPI","Variables.htm#ScreenDPI",1],
  ["A_ScreenHeight","Variables.htm#Screen",1],
  ["A_ScreenWidth","Variables.htm#Screen",1],
  ["A_ScriptDir","Variables.htm#ScriptDir",1],
  ["A_ScriptFullPath","Variables.htm#ScriptFullPath",1],
  ["A_ScriptHwnd","Variables.htm#ScriptHwnd",1],
  ["A_ScriptName","Variables.htm#ScriptName",1],
  ["A_Sec","Variables.htm#Sec",1],
  ["A_SendLevel","Variables.htm#SendLevel",1],
  ["A_SendMode","Variables.htm#SendMode",1],
  ["A_Space","Variables.htm#Space",1],
  ["A_StartMenu","Variables.htm#StartMenu",1],
  ["A_StartMenuCommon","Variables.htm#StartMenuCommon",1],
  ["A_Startup","Variables.htm#Startup",1],
  ["A_StartupCommon","Variables.htm#StartupCommon",1],
  ["A_StoreCapsLockMode","Variables.htm#StoreCapsLockMode",1],
  ["A_Tab","Variables.htm#Tab",1],
  ["A_Temp","Variables.htm#Temp",1],
  ["A_ThisFunc","Variables.htm#ThisFunc",1],
  ["A_ThisHotkey","Variables.htm#ThisHotkey",1],
  ["A_TickCount","Variables.htm#TickCount",1],
  ["A_TimeIdle","Variables.htm#TimeIdle",1],
  ["A_TimeIdleKeyboard","Variables.htm#TimeIdleKeyboard",1],
  ["A_TimeIdleMouse","Variables.htm#TimeIdleMouse",1],
  ["A_TimeIdlePhysical","Variables.htm#TimeIdlePhysical",1],
  ["A_TimeSincePriorHotkey","Variables.htm#TimeSincePriorHotkey",1],
  ["A_TimeSinceThisHotkey","Variables.htm#TimeSinceThisHotkey",1],
  ["A_TitleMatchMode","Variables.htm#TitleMatchMode",1],
  ["A_TitleMatchModeSpeed","Variables.htm#TitleMatchModeSpeed",1],
  ["A_TrayMenu","Variables.htm#TrayMenu",1],
  ["A_UserName","Variables.htm#UserName",1],
  ["A_WDay","Variables.htm#WDay",1],
  ["A_WinDelay","Variables.htm#WinDelay",1],
  ["A_WinDir","Variables.htm#WinDir",1],
  ["A_WorkFileName (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#WorkFileName",99],
  ["A_WorkingDir","Variables.htm#WorkingDir",1],
  ["A_YDay","Variables.htm#YDay",1],
  ["A_Year","Variables.htm#YYYY",1],
  ["A_YWeek","Variables.htm#YWeek",1],
  ["A_YYYY","Variables.htm#YYYY",1],
  ["abbreviation expansion","Hotstrings.htm"],
  ["Abs","commands/Math.htm#Abs",2],
  ["absolute value, Abs","commands/Math.htm#Abs"],
  ["Acknowledgements","misc/Acknowledgements.htm"],
  ["ACos","commands/Math.htm#ACos",2],
  ["activate a window","commands/WinActivate.htm"],
  ["ActiveX controls (GUI)","commands/GuiControls.htm#ActiveX"],
  ["Add method (Gui)","objects/Gui.htm#Add",7],
  ["Add method (GuiControl)","objects/GuiControl.htm#Add",7],
  ["Add method (ListView)","commands/ListView.htm#Add",7],
  ["Add method (Menu)","objects/Menu.htm#Add",7],
  ["Add method (TreeView)","commands/TreeView.htm#Add",7],
  ["address of a variable","commands/StrPtr.htm"],
  ["AddResource directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#AddResource",99],
  ["AddStandard method (Menu)","objects/Menu.htm#AddStandard",7],
  ["administrator privileges for scripts","Variables.htm#RequireAdmin"],
  ["Ahk2Exe","Scripts.htm#ahk2exe"],
  ["ahk_class","misc/WinTitle.htm#ahk_class"],
  ["ahk_exe","misc/WinTitle.htm#ahk_exe"],
  ["ahk_group","misc/WinTitle.htm#ahk_group"],
  ["ahk_id","misc/WinTitle.htm#ahk_id"],
  ["ahk_pid","misc/WinTitle.htm#ahk_pid"],
  ["alnum","commands/Is.htm"],
  ["alpha","commands/Is.htm"],
  ["AltGr","Hotkeys.htm#AltGr"],
  ["AltTab","Hotkeys.htm#alttab"],
  ["and","Variables.htm#and",4],
  ["Any","objects/Any.htm",6],
  ["append to file","commands/FileAppend.htm"],
  ["Array","objects/Array.htm",6],
  ["ASin","commands/Math.htm#ASin",2],
  ["assigning values to variables","Variables.htm#AssignOp"],
  ["ATan","commands/Math.htm#ATan",2],
  ["AtEOF property (File)","objects/File.htm#AtEOF",7],
  ["attributes of files and folders","commands/FileGetAttrib.htm"],
  ["auto-execute section","Scripts.htm#auto"],
  ["auto-replace text as you type it","Hotstrings.htm"],
  ["BackColor property (Gui)","objects/Gui.htm#BackColor",7],
  ["BackspaceIsUndo property (InputHook)","commands/InputHook.htm#BackspaceIsUndo",7],
  ["balloon tip","commands/TrayTip.htm"],
  ["base (Objects)","Objects.htm#Custom_Objects"],
  ["Base directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Bin",99],
  ["Base property (Any)","objects/Any.htm#Base",7],
  ["Base property (Object)","objects/Object.htm#Base",7],
  ["beep the PC speaker","commands/SoundBeep.htm"],
  ["Bin directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Bin",99],
  ["Bind method (Func)","objects/Func.htm#Bind",7],
  ["BinMod script (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#PostExec",99],
  ["bitwise operations","Variables.htm#bitshift"],
  ["blind-mode Send","commands/Send.htm#blind"],
  ["BlockInput","commands/BlockInput.htm",2],
  ["blocks (lines enclosed in braces)","commands/Block.htm"],
  ["boolean (concepts)","Concepts.htm#boolean"],
  ["BoundFunc","objects/Functor.htm#BoundFunc",6],
  ["Break","commands/Break.htm",3,"S"],
  ["Buffer","objects/Buffer.htm",6],
  ["buffering","commands/_MaxThreadsBuffer.htm"],
  ["built-in functions","Functions.htm#BuiltIn"],
  ["built-in variables","Variables.htm#BuiltIn"],
  ["Button controls (GUI)","commands/GuiControls.htm#Button"],
  ["button list (mouse and joystick)","KeyList.htm"],
  ["button state","commands/GetKeyState.htm"],
  ["ByRef","Functions.htm#ByRef",4],
  ["caching (concepts)","Concepts.htm#caching"],
  ["Call method (Array)","objects/Array.htm#Call",7],
  ["Call method (Buffer)","objects/Buffer.htm#Call",7],
  ["Call method (Class)","objects/Class.htm#Call",7],
  ["Call method (Enumerator)","objects/Enumerator.htm#Call",7],
  ["Call method (Func)","objects/Func.htm#Call",7],
  ["Call method (Gui)","objects/Gui.htm#Call",7],
  ["Call method (Map)","objects/Map.htm#Call",7],
  ["Call method (Menu)","objects/Menu.htm#Call",7],
  ["Call method (Object)","objects/Object.htm#Call",7],
  ["CallbackCreate","commands/CallbackCreate.htm",2],
  ["CallbackFree","commands/CallbackCreate.htm#CallbackFree",2],
  ["Capacity property (Array)","objects/Array.htm#Capacity",7],
  ["Capacity property (Map)","objects/Map.htm#Capacity",7],
  ["CaretGetPos","commands/CaretGetPos.htm",2],
  ["Case","commands/Switch.htm",3,"E"],
  ["CaseSense property (Map)","objects/Map.htm#CaseSense",7],
  ["CaseSensitive property (InputHook)","commands/InputHook.htm#CaseSensitive",7],
  ["Catch","commands/Catch.htm",3,""],
  ["Catch ... as","commands/Catch.htm",3,""],
  ["Ceil","commands/Math.htm#Ceil",2],
  ["Changelog","ChangeLog.htm"],
  ["Check method (Menu)","objects/Menu.htm#Check",7],
  ["Checkbox controls (GUI)","commands/GuiControls.htm#Checkbox"],
  ["choose file","commands/FileSelect.htm"],
  ["choose folder","commands/DirSelect.htm"],
  ["Choose method (GuiControl)","objects/GuiControl.htm#Choose",7],
  ["Chr","commands/Chr.htm",2],
  ["class","Objects.htm#Custom_Classes",5],
  ["Class","objects/Class.htm",6],
  ["class ... extends","Objects.htm#Custom_Classes",5],
  ["class name of a window","commands/WinGetClass.htm"],
  ["Class object","objects/Class.htm"],
  ["ClassNN (of a control)","commands/ControlGetClassNN.htm"],
  ["ClassNN property (GuiControl)","objects/GuiControl.htm#ClassNN",7],
  ["Clear method (Map)","objects/Map.htm#Clear",7],
  ["Click","commands/Click.htm",2],
  ["Click a mouse button","commands/Click.htm"],
  ["ClickCount property (Menu)","objects/Menu.htm#ClickCount",7],
  ["Clipboard","misc/A_Clipboard.htm"],
  ["ClipboardAll","commands/ClipboardAll.htm",6],
  ["ClipWait","commands/ClipWait.htm",2],
  ["Clone method (Array)","objects/Array.htm#Clone",7],
  ["Clone method (Map)","objects/Map.htm#Clone",7],
  ["Clone method (Object)","objects/Object.htm#Clone",7],
  ["close a window","commands/WinClose.htm"],
  ["Close (Gui event)","objects/GuiOnEvent.htm#Close"],
  ["Close method (File)","objects/File.htm#Close",7],
  ["Closure","Functions.htm#closures",6],
  ["CLSID List (My Computer, etc.)","misc/CLSID-List.htm"],
  ["coalescing operator","Variables.htm#or-maybe"],
  ["color names, RGB/HTML","misc/Colors.htm"],
  ["color of pixels","commands/PixelSearch.htm"],
  ["COM","commands/ComObject.htm"],
  ["ComboBox controls (GUI)","commands/GuiControls.htm#ComboBox"],
  ["comma operator (multi-statement)","Variables.htm#comma"],
  ["command line parameters","Scripts.htm#cmd"],
  ["comments in scripts","Language.htm#comments"],
  ["ComCall","commands/ComCall.htm",2],
  ["ComObjActive","commands/ComObjActive.htm",2],
  ["ComObjArray","commands/ComObjArray.htm",6],
  ["ComObjConnect","commands/ComObjConnect.htm",2],
  ["ComObject","commands/ComObject.htm",6],
  ["ComObjFlags","commands/ComObjFlags.htm",2],
  ["ComObjFromPtr","commands/ComObjFromPtr.htm",2],
  ["ComObjGet","commands/ComObjGet.htm",2],
  ["ComObjQuery","commands/ComObjQuery.htm",2],
  ["ComObjType","commands/ComObjType.htm",2],
  ["ComObjValue","commands/ComObjValue.htm",2],
  ["ComValue","commands/ComValue.htm",6],
  ["ComValueRef","commands/ComValue.htm",6],
  ["Compatibility","Compat.htm"],
  ["compile a script","Scripts.htm#ahk2exe"],
  ["Compiler Directives","misc/Ahk2ExeDirectives.htm"],
  ["Compression","Scripts.htm#mpress"],
  ["concatenate, in expressions","Variables.htm#concat"],
  ["concatenate, script lines","Scripts.htm#continuation"],
  ["concepts and conventions","Concepts.htm"],
  ["ConsoleApp directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#ConsoleApp",99],
  ["Cont directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Cont",99],
  ["ContextMenu (Gui event)","objects/GuiOnEvent.htm#ContextMenu"],
  ["continuation sections","Scripts.htm#continuation"],
  ["Continue","commands/Continue.htm",3,"S"],
  ["control flow","Language.htm#control-flow"],
  ["Control functions","commands/Control.htm"],
  ["ControlAddItem","commands/ControlAddItem.htm",2],
  ["ControlChooseIndex","commands/ControlChooseIndex.htm",2],
  ["ControlChooseString","commands/ControlChooseString.htm",2],
  ["ControlClick","commands/ControlClick.htm",2],
  ["ControlDeleteItem","commands/ControlDeleteItem.htm",2],
  ["ControlFindItem","commands/ControlFindItem.htm",2],
  ["ControlFocus","commands/ControlFocus.htm",2],
  ["ControlGetChecked","commands/ControlGetChecked.htm",2],
  ["ControlGetChoice","commands/ControlGetChoice.htm",2],
  ["ControlGetClassNN","commands/ControlGetClassNN.htm",2],
  ["ControlGetEnabled","commands/ControlGetEnabled.htm",2],
  ["ControlGetExStyle","commands/ControlGetStyle.htm",2],
  ["ControlGetFocus","commands/ControlGetFocus.htm",2],
  ["ControlGetHwnd","commands/ControlGetHwnd.htm",2],
  ["ControlGetIndex","commands/ControlGetIndex.htm",2],
  ["ControlGetItems","commands/ControlGetItems.htm",2],
  ["ControlGetPos","commands/ControlGetPos.htm",2],
  ["ControlGetStyle","commands/ControlGetStyle.htm",2],
  ["ControlGetText","commands/ControlGetText.htm",2],
  ["ControlGetVisible","commands/ControlGetVisible.htm",2],
  ["ControlHide","commands/ControlHide.htm",2],
  ["ControlHideDropDown","commands/ControlHideDropDown.htm",2],
  ["ControlMove","commands/ControlMove.htm",2],
  ["ControlSend","commands/ControlSend.htm",2],
  ["ControlSendText","commands/ControlSend.htm",2],
  ["ControlSetChecked","commands/ControlSetChecked.htm",2],
  ["ControlSetEnabled","commands/ControlSetEnabled.htm",2],
  ["ControlSetExStyle","commands/ControlSetStyle.htm",2],
  ["ControlSetStyle","commands/ControlSetStyle.htm",2],
  ["ControlSetText","commands/ControlSetText.htm",2],
  ["ControlShow","commands/ControlShow.htm",2],
  ["ControlShowDropDown","commands/ControlShowDropDown.htm",2],
  ["convert a script to an EXE","Scripts.htm#ahk2exe"],
  ["coordinates","commands/CoordMode.htm"],
  ["CoordMode","commands/CoordMode.htm",2],
  ["copy file","commands/FileCopy.htm"],
  ["copy folder/directory","commands/DirCopy.htm"],
  ["Cos","commands/Math.htm#Cos",2],
  ["Count property (Map)","objects/Map.htm#Count",7],
  ["Count property (Match)","commands/RegExMatch.htm#MatchObject",7],
  ["create file","commands/FileAppend.htm"],
  ["create folder/directory","commands/DirCreate.htm"],
  ["Critical","commands/Critical.htm",2],
  ["current directory","commands/SetWorkingDir.htm"],
  ["current thread","misc/Threads.htm"],
  ["cursor shape","Variables.htm#Cursor"],
  ["custom combination hotkeys","Hotkeys.htm#combo"],
  ["Custom controls (GUI)","commands/GuiControls.htm#Custom"],
  ["Dash","Program.htm#dash"],
  ["DateAdd","commands/DateAdd.htm",2],
  ["DateDiff","commands/DateDiff.htm",2],
  ["dates and times (of files)","commands/FileSetTime.htm"],
  ["DateTime controls (GUI)","commands/GuiControls.htm#DateTime"],
  ["Debug directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Debug",99],
  ["debugger","commands/OutputDebug.htm"],
  ["debugging a script","Scripts.htm#debug"],
  ["decimal places","commands/Format.htm"],
  ["Default","commands/Switch.htm",3,""],
  ["Default property (Array)","objects/Array.htm#Default",7],
  ["Default property (Map)","objects/Map.htm#Default",7],
  ["Default property (Menu)","objects/Menu.htm#Default",7],
  ["DefineProp method (Object)","objects/Object.htm#DefineProp",7],
  ["delete files","commands/FileDelete.htm"],
  ["delete folder/directory","commands/DirDelete.htm"],
  ["Delete method (Array)","objects/Array.htm#Delete",7],
  ["Delete method (GuiControl)","objects/GuiControl.htm#Delete",7],
  ["Delete method (ListView)","commands/ListView.htm#Delete",7],
  ["Delete method (Map)","objects/Map.htm#Delete",7],
  ["Delete method (Menu)","objects/Menu.htm#Delete",7],
  ["Delete method (TreeView)","commands/TreeView.htm#Delete",7],
  ["DeleteCol method (ListView)","commands/ListView.htm#DeleteCol",7],
  ["DeleteProp method (Object)","objects/Object.htm#DeleteProp",7],
  ["dereference","Variables.htm#deref"],
  ["Destroy method (Gui)","objects/Gui.htm#Destroy",7],
  ["DetectHiddenText","commands/DetectHiddenText.htm",2],
  ["DetectHiddenWindows","commands/DetectHiddenWindows.htm",2],
  ["dialog DirSelect","commands/DirSelect.htm"],
  ["dialog FileSelect","commands/FileSelect.htm"],
  ["dialog InputBox","commands/InputBox.htm"],
  ["dialog MsgBox","commands/MsgBox.htm"],
  ["digit","commands/Is.htm"],
  ["DirCopy","commands/DirCopy.htm",2],
  ["DirCreate","commands/DirCreate.htm",2],
  ["DirDelete","commands/DirDelete.htm",2],
  ["DirExist","commands/DirExist.htm",2],
  ["DirMove","commands/DirMove.htm",2],
  ["DirSelect","commands/DirSelect.htm",2],
  ["Disable method (Menu)","objects/Menu.htm#Disable",7],
  ["disk space","commands/DriveGetSpaceFree.htm"],
  ["divide (math)","Variables.htm#divide"],
  ["DllCall","commands/DllCall.htm",2],
  ["double-deref","Variables.htm#deref"],
  ["Download","commands/Download.htm",2],
  ["DPI scaling","misc/DPIScaling.htm"],
  ["drag and drop (GUI windows)","objects/GuiOnEvent.htm#DropFiles"],
  ["drag the mouse","commands/MouseClickDrag.htm"],
  ["Drive functions","commands/Drive.htm"],
  ["DriveEject","commands/DriveEject.htm",2],
  ["DriveGetCapacity","commands/DriveGetCapacity.htm",2],
  ["DriveGetFileSystem","commands/DriveGetFileSystem.htm",2],
  ["DriveGetLabel","commands/DriveGetLabel.htm",2],
  ["DriveGetList","commands/DriveGetList.htm",2],
  ["DriveGetSerial","commands/DriveGetSerial.htm",2],
  ["DriveGetSpaceFree","commands/DriveGetSpaceFree.htm",2],
  ["DriveGetStatus","commands/DriveGetStatus.htm",2],
  ["DriveGetStatusCD","commands/DriveGetStatusCD.htm",2],
  ["DriveLock","commands/DriveLock.htm",2],
  ["DriveRetract","commands/DriveEject.htm",2],
  ["DriveSetLabel","commands/DriveSetLabel.htm",2],
  ["DriveUnlock","commands/DriveUnlock.htm",2],
  ["DriveGetType","commands/DriveGetType.htm",2],
  ["DropDownList controls (GUI)","commands/GuiControls.htm#DropDownList"],
  ["DropFiles (Gui event)","objects/GuiOnEvent.htm#DropFiles"],
  ["Dynamic function calls","Functions.htm#DynCall"],
  ["dynamic variables","Language.htm#dynamic-variables"],
  ["Edit","commands/Edit.htm",2],
  ["Edit controls (GUI)","commands/GuiControls.htm#Edit"],
  ["EditGetCurrentCol","commands/EditGetCurrentCol.htm",2],
  ["EditGetCurrentLine","commands/EditGetCurrentLine.htm",2],
  ["EditGetLine","commands/EditGetLine.htm",2],
  ["EditGetLineCount","commands/EditGetLineCount.htm",2],
  ["EditGetSelectedText","commands/EditGetSelectedText.htm",2],
  ["EditPaste","commands/EditPaste.htm",2],
  ["Else","commands/Else.htm",3,""],
  ["Enable method (Menu)","objects/Menu.htm#Enable",7],
  ["Enabled property (GuiControl)","objects/GuiControl.htm#Enabled",7],
  ["Encoding property (File)","objects/File.htm#Encoding",7],
  ["EndKey property (InputHook)","commands/InputHook.htm#EndKey",7],
  ["EndMods property (InputHook)","commands/InputHook.htm#EndMods",7],
  ["EndReason property (InputHook)","commands/InputHook.htm#EndReason",7],
  ["Enumerator","objects/Enumerator.htm",6],
  ["EnvGet","commands/EnvGet.htm",2],
  ["environment variables","Concepts.htm#environment-variables"],
  ["environment variables (change them)","commands/EnvSet.htm"],
  ["EnvSet","commands/EnvSet.htm",2],
  ["Error","objects/Error.htm",6],
  ["ErrorStdOut","commands/_ErrorStdOut.htm"],
  ["escape sequence","misc/EscapeChar.htm"],
  ["Escape (Gui event)","objects/GuiOnEvent.htm#Escape"],
  ["ExeName directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#ExeName",99],
  ["Exit","commands/Exit.htm",2],
  ["ExitApp","commands/ExitApp.htm",2],
  ["Exp","commands/Math.htm#Exp",2],
  ["expressions","Variables.htm#Expressions"],
  ["Extra property (Error)","objects/Error.htm#Extra",7],
  ["False","Variables.htm#Boolean",1],
  ["fat arrow functions","Variables.htm#fat-arrow"],
  ["FAQ (Frequently Asked Questions)","FAQ.htm"],
  ["File","objects/File.htm",6],
  ["file attributes","commands/FileSetAttrib.htm"],
  ["file or folder (does it exist)","commands/FileExist.htm"],
  ["file pattern","commands/LoopFiles.htm"],
  ["File property (Error)","objects/Error.htm#File",7],
  ["file, creating","commands/FileAppend.htm"],
  ["file, reading","commands/LoopRead.htm"],
  ["file, writing/appending","commands/FileAppend.htm"],
  ["FileAppend","commands/FileAppend.htm",2],
  ["FileCopy","commands/FileCopy.htm",2],
  ["FileCreateShortcut","commands/FileCreateShortcut.htm",2],
  ["FileDelete","commands/FileDelete.htm",2],
  ["FileEncoding","commands/FileEncoding.htm",2],
  ["FileExist","commands/FileExist.htm",2],
  ["FileGetAttrib","commands/FileGetAttrib.htm",2],
  ["FileGetShortcut","commands/FileGetShortcut.htm",2],
  ["FileGetSize","commands/FileGetSize.htm",2],
  ["FileGetTime","commands/FileGetTime.htm",2],
  ["FileGetVersion","commands/FileGetVersion.htm",2],
  ["FileInstall","commands/FileInstall.htm",2],
  ["FileMove","commands/FileMove.htm",2],
  ["FileOpen","commands/FileOpen.htm",2],
  ["FileRead","commands/FileRead.htm",2],
  ["FileRecycle","commands/FileRecycle.htm",2],
  ["FileRecycleEmpty","commands/FileRecycleEmpty.htm",2],
  ["FileSelect","commands/FileSelect.htm",2],
  ["FileSetAttrib","commands/FileSetAttrib.htm",2],
  ["FileSetTime","commands/FileSetTime.htm",2],
  ["Finally","commands/Finally.htm",3,""],
  ["find a file","commands/LoopFiles.htm"],
  ["find a string","commands/InStr.htm"],
  ["find a window","commands/WinExist.htm"],
  ["FindAnywhere property (InputHook)","commands/InputHook.htm#FindAnywhere",7],
  ["Flash method (Gui)","objects/Gui.htm#Flash",7],
  ["Float","commands/Float.htm",2],
  ["Float","Objects.htm#primitive",6],
  ["floating point (check if it is one)","commands/Is.htm"],
  ["floating point (Format)","commands/Format.htm"],
  ["Floor","commands/Math.htm#Floor",2],
  ["focus","commands/ControlFocus.htm"],
  ["Focus method (GuiControl)","objects/GuiControl.htm#Focus",7],
  ["Focused property (GuiControl)","objects/GuiControl.htm#Focused",7],
  ["FocusedCtrl property (Gui)","objects/Gui.htm#FocusedCtrl",7],
  ["folder/directory copy","commands/DirCopy.htm"],
  ["folder/directory create","commands/DirCreate.htm"],
  ["folder/directory move","commands/DirMove.htm"],
  ["folder/directory remove","commands/DirDelete.htm"],
  ["folder/directory select","commands/DirSelect.htm"],
  ["Fonts","misc/FontsStandard.htm"],
  ["For ... in","commands/For.htm",3,"IE"],
  ["Format","commands/Format.htm",2],
  ["FormatTime","commands/FormatTime.htm",2],
  ["free space","commands/DriveGetSpaceFree.htm"],
  ["FTP uploading example","commands/FileAppend.htm#ExFTP"],
  ["Func","objects/Func.htm",6],
  ["functions, alphabetical list","commands/index.htm"],
  ["functions (concepts)","Concepts.htm#functions"],
  ["functions (defining and calling)","Functions.htm"],
  ["function objects","objects/Functor.htm"],
  ["game automation","commands/PixelSearch.htm"],
  ["Get method (Array)","objects/Array.htm#Get",7],
  ["Get method (Map)","objects/Map.htm#Get",7],
  ["Get method (TreeView)","commands/TreeView.htm#Get",7],
  ["GetChild method (TreeView)","commands/TreeView.htm#GetChild",7],
  ["GetClientPos method (Gui)","objects/Gui.htm#GetClientPos",7],
  ["GetCount method (ListView)","commands/ListView.htm#GetCount",7],
  ["GetCount method (TreeView)","commands/TreeView.htm#GetCount",7],
  ["GetKeyName","commands/GetKeyName.htm",2],
  ["GetKeySC","commands/GetKeySC.htm",2],
  ["GetKeyState","commands/GetKeyState.htm",2],
  ["GetKeyVK","commands/GetKeyVK.htm",2],
  ["GetMethod","commands/GetMethod.htm",2],
  ["GetMethod method (Any)","objects/Any.htm#GetMethod",7],
  ["GetNext method (ListView)","commands/ListView.htm#GetNext",7],
  ["GetNext method (TreeView)","commands/TreeView.htm#GetNext",7],
  ["GetOwnPropDesc method (Object)","objects/Object.htm#GetOwnPropDesc",7],
  ["GetParent method (TreeView)","commands/TreeView.htm#GetParent",7],
  ["GetPos method (Gui)","objects/Gui.htm#GetPos",7],
  ["GetPos method (GuiControl)","objects/GuiControl.htm#GetPos",7],
  ["GetPrev method (TreeView)","commands/TreeView.htm#GetPrev",7],
  ["GetSelection method (TreeView)","commands/TreeView.htm#GetSelection",7],
  ["GetText method (ListView)","commands/ListView.htm#GetText",7],
  ["GetText method (TreeView)","commands/TreeView.htm#GetText",7],
  ["global","Functions.htm#Global",5],
  ["global code","Language.htm#global-code"],
  ["global variables in functions","Functions.htm#Global"],
  ["Goto","commands/Goto.htm",3,"S"],
  ["GroupActivate","commands/GroupActivate.htm",2],
  ["GroupAdd","commands/GroupAdd.htm",2],
  ["GroupBox controls (GUI)","commands/GuiControls.htm#GroupBox"],
  ["GroupClose","commands/GroupClose.htm",2],
  ["GroupDeactivate","commands/GroupDeactivate.htm",2],
  ["Gui","objects/Gui.htm",6],
  ["Gui (create a GUI)","objects/Gui.htm#Call"],
  ["Gui control types","commands/GuiControls.htm"],
  ["Gui events","objects/GuiOnEvent.htm"],
  ["Gui property (GuiControl)","objects/GuiControl.htm#Gui",7],
  ["Gui styles reference","misc/Styles.htm"],
  ["GuiControl object","objects/GuiControl.htm"],
  ["GuiCtrlFromHwnd","commands/GuiCtrlFromHwnd.htm",2],
  ["GuiFromHwnd","commands/GuiFromHwnd.htm",2],
  ["Handle property (File)","objects/File.htm#Handle",7],
  ["Handle property (Menu)","objects/Menu.htm#Handle",7],
  ["Has method (Array)","objects/Array.htm#Has",7],
  ["Has method (Map)","objects/Map.htm#Has",7],
  ["HasBase","commands/HasBase.htm",2],
  ["HasBase method (Any)","objects/Any.htm#HasBase",7],
  ["HasMethod","commands/HasMethod.htm",2],
  ["HasMethod method (Any)","objects/Any.htm#HasMethod",7],
  ["HasOwnProp method (Object)","objects/Object.htm#HasOwnProp",7],
  ["HasProp","commands/HasProp.htm",2],
  ["HasProp method (Any)","objects/Any.htm#HasProp",7],
  ["HBITMAP:","misc/ImageHandles.htm"],
  ["hexadecimal format","commands/Format.htm"],
  ["hibernate or suspend","commands/Shutdown.htm#ExSuspend"],
  ["HICON:","misc/ImageHandles.htm"],
  ["hidden text","commands/DetectHiddenText.htm"],
  ["hidden windows","commands/DetectHiddenWindows.htm"],
  ["Hide method (Gui)","objects/Gui.htm#Hide",7],
  ["HKEY_CLASSES_ROOT (HKCR)","commands/RegRead.htm"],
  ["HKEY_CURRENT_CONFIG (HKCC)","commands/RegRead.htm"],
  ["HKEY_CURRENT_USER (HKCU)","commands/RegRead.htm"],
  ["HKEY_LOCAL_MACHINE (HKLM)","commands/RegRead.htm"],
  ["HKEY_USERS (HKU)","commands/RegRead.htm"],
  ["hook","commands/InstallKeybdHook.htm"],
  ["HotIf","commands/HotIf.htm",2],
  ["HotIfWinActive","commands/HotIf.htm#IfWin",2],
  ["HotIfWinExist","commands/HotIf.htm#IfWin",2],
  ["HotIfWinNotActive","commands/HotIf.htm#IfWin",2],
  ["HotIfWinNotExist","commands/HotIf.htm#IfWin",2],
  ["Hotkey","commands/Hotkey.htm",2],
  ["Hotkeys (general information)","Hotkeys.htm"],
  ["Hotkey controls (GUI)","commands/GuiControls.htm#Hotkey"],
  ["Hotkey, ListHotkeys","commands/ListHotkeys.htm"],
  ["Hotkey, other features","HotkeyFeatures.htm"],
  ["Hotstring","commands/Hotstring.htm",2],
  ["Hotstrings (general information)","Hotstrings.htm"],
  ["HTML color names","misc/Colors.htm"],
  ["HWND (of a control)","commands/ControlGetHwnd.htm"],
  ["HWND (of a window)","misc/WinTitle.htm#ahk_id"],
  ["Hwnd property (Gui)","objects/Gui.htm#Hwnd",7],
  ["Hwnd property (GuiControl)","objects/GuiControl.htm#Hwnd",7],
  ["icon, changing","commands/TraySetIcon.htm"],
  ["ID number for a window","commands/WinExist.htm"],
  ["If","commands/If.htm",3,"E"],
  ["IL_Add","commands/ListView.htm#IL_Add",2],
  ["IL_Create","commands/ListView.htm#IL_Create",2],
  ["IL_Destroy","commands/ListView.htm#IL_Destroy",2],
  ["IgnoreBegin directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#IgnoreKeep",99],
  ["IgnoreEnd directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#IgnoreKeep",99],
  ["Image Lists (GUI)","commands/ListView.htm#IL"],
  ["ImageSearch","commands/ImageSearch.htm",2],
  ["Include","commands/_Include.htm"],
  ["IndexError","objects/Error.htm#IndexError",6],
  ["infrared remote controls","scripts/index.htm#WinLIRC"],
  ["IniDelete","commands/IniDelete.htm",2],
  ["IniRead","commands/IniRead.htm",2],
  ["IniWrite","commands/IniWrite.htm",2],
  ["InProgress property (InputHook)","commands/InputHook.htm#InProgress",7],
  ["Input property (InputHook)","commands/InputHook.htm#Input",7],
  ["InputBox","commands/InputBox.htm",2],
  ["InputHook","commands/InputHook.htm",6],
  ["Insert method (ListView)","commands/ListView.htm#Insert",7],
  ["Insert method (Menu)","objects/Menu.htm#Insert",7],
  ["InsertAt method (Array)","objects/Array.htm#InsertAt",7],
  ["InsertCol method (ListView)","commands/ListView.htm#InsertCol",7],
  ["Install","commands/FileInstall.htm"],
  ["installer options","Program.htm#install"],
  ["InstallKeybdHook","commands/InstallKeybdHook.htm",2],
  ["InstallMouseHook","commands/InstallMouseHook.htm",2],
  ["InStr","commands/InStr.htm",2],
  ["Integer","commands/Integer.htm",2],
  ["Integer","commands/Integer.htm",6],
  ["integer (check if it is one)","commands/Is.htm"],
  ["integer (Format)","commands/Format.htm"],
  ["Interrupt (Thread)","commands/Thread.htm#Interrupt"],
  ["is","Variables.htm#is",4],
  ["IsAlnum","commands/Is.htm#alnum",2],
  ["IsAlpha","commands/Is.htm#alpha",2],
  ["IsBuiltIn property (Func)","objects/Func.htm#IsBuiltIn",7],
  ["IsByRef method (Func)","objects/Func.htm#IsByRef",7],
  ["IsDigit","commands/Is.htm#digit",2],
  ["IsFloat","commands/Is.htm#float",2],
  ["IsInteger","commands/Is.htm#integer",2],
  ["IsLabel","commands/IsLabel.htm",2],
  ["IsLower","commands/Is.htm#lower",2],
  ["IsNumber","commands/Is.htm#number",2],
  ["IsObject","commands/IsObject.htm",2],
  ["IsOptional method (Func)","objects/Func.htm#IsOptional",7],
  ["IsSet","commands/IsSet.htm",2],
  ["IsSetRef","commands/IsSet.htm",2],
  ["IsSpace","commands/Is.htm#space",2],
  ["IsTime","commands/Is.htm#time",2],
  ["IsUpper","commands/Is.htm#upper",2],
  ["IsVariadic property (Func)","objects/Func.htm#IsVariadic",7],
  ["IsXDigit","commands/Is.htm#xdigit",2],
  ["Join (continuation sections)","Scripts.htm#Join"],
  ["Joystick","KeyList.htm#Joystick"],
  ["JScript, embedded/inline","commands/DllCall.htm#COM"],
  ["Keep directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#IgnoreKeep",99],
  ["key list (keyboard, mouse, joystick)","KeyList.htm"],
  ["key state","commands/GetKeyState.htm"],
  ["keyboard hook","commands/InstallKeybdHook.htm"],
  ["KeyHistory","commands/KeyHistory.htm",2],
  ["KeyOpt method (InputHook)","commands/InputHook.htm#KeyOpt",7],
  ["keystrokes, sending","commands/Send.htm"],
  ["KeyWait","commands/KeyWait.htm",2],
  ["labels","misc/Labels.htm"],
  ["language overview","Language.htm"],
  ["last found window","misc/WinTitle.htm#LastFoundWindow"],
  ["Launcher","Program.htm#launcher"],
  ["Len method/property (Match)","commands/RegExMatch.htm#MatchObject",7],
  ["length of a string","commands/StrLen.htm"],
  ["Length property (Array)","objects/Array.htm#Length",7],
  ["Length property (File)","objects/File.htm#Length",7],
  ["Let directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Let",99],
  ["library (Lib) folders","Scripts.htm#lib"],
  ["license","license.htm"],
  ["line continuation","Scripts.htm#continuation"],
  ["Line property (Error)","objects/Error.htm#Line",7],
  ["Link controls (GUI)","commands/GuiControls.htm#Link"],
  ["ListBox controls (GUI)","commands/GuiControls.htm#ListBox"],
  ["ListHotkeys","commands/ListHotkeys.htm",2],
  ["ListLines","commands/ListLines.htm",2],
  ["ListVars","commands/ListVars.htm",2],
  ["ListView controls (GUI)","commands/ListView.htm"],
  ["ListViewGetContent","commands/ListViewGetContent.htm",2],
  ["Ln","commands/Math.htm#Ln",2],
  ["lnk (link/shortcut) file","commands/FileCreateShortcut.htm"],
  ["LoadPicture","commands/LoadPicture.htm",2],
  ["local","Functions.htm#Local",5],
  ["local variables","Functions.htm#Local"],
  ["Locale","commands/StrCompare.htm#Locale"],
  ["Log","commands/Math.htm#Log",2],
  ["logarithm, Log","commands/Math.htm#Log"],
  ["logoff","commands/Shutdown.htm"],
  ["long file name (converting to)","commands/LoopFiles.htm#LoopFilePath"],
  ["long paths","misc/LongPaths.htm"],
  ["Loop","commands/Loop.htm",3,"E"],
  ["Loop (until)","commands/Until.htm"],
  ["Loop (while)","commands/While.htm"],
  ["Loop Files","commands/LoopFiles.htm",3,"EE"],
  ["Loop Parse","commands/LoopParse.htm",3,"EEE"],
  ["Loop Read","commands/LoopRead.htm",3,"EE"],
  ["Loop Reg","commands/LoopReg.htm",3,"EE"],
  ["lParam","commands/SendMessage.htm"],
  ["LTrim (continuation sections)","Scripts.htm#LTrim"],
  ["LTrim","commands/Trim.htm",2],
  ["macro","misc/Macros.htm"],
  ["main window","Program.htm#main-window"],
  ["Map","objects/Map.htm",6],
  ["MarginX property (Gui)","objects/Gui.htm#MarginX",7],
  ["MarginY property (Gui)","objects/Gui.htm#MarginY",7],
  ["Mark property (Match)","commands/RegExMatch.htm#MatchObject",7],
  ["Match property (InputHook)","commands/InputHook.htm#Match",7],
  ["math functions","commands/Math.htm"],
  ["math operations (expressions)","Variables.htm#Expressions"],
  ["Max","commands/Math.htm#Max",2],
  ["Maximize method (Gui)","objects/Gui.htm#Maximize",7],
  ["MaxParams property (Func)","objects/Func.htm#MaxParams",7],
  ["MaxThreads","commands/_MaxThreads.htm"],
  ["MaxThreadsBuffer","commands/_MaxThreadsBuffer.htm"],
  ["MaxThreadsPerHotkey","commands/_MaxThreadsPerHotkey.htm"],
  ["maybe operator","Variables.htm#maybe"],
  ["Menu","objects/Menu.htm",6],
  ["MenuBar","objects/Menu.htm",6],
  ["MenuBar property (Gui)","objects/Gui.htm#MenuBar",7],
  ["MenuFromHandle","commands/MenuFromHandle.htm",2],
  ["MenuSelect","commands/MenuSelect.htm",2],
  ["MemberError","objects/Error.htm#MemberError",6],
  ["MemoryError","objects/Error.htm#MemoryError",6],
  ["message list (WM_*)","misc/SendMessageList.htm"],
  ["Message property (Error)","objects/Error.htm#Message",7],
  ["messages, receiving","commands/OnMessage.htm"],
  ["messages, posting","commands/PostMessage.htm"],
  ["messages, sending","commands/SendMessage.htm"],
  ["meta-functions (Objects)","Objects.htm#Meta_Functions"],
  ["MethodError","objects/Error.htm#MemberError",6],
  ["methods (concepts)","Concepts.htm#methods"],
  ["Min","commands/Math.htm#Min",2],
  ["Minimize method (Gui)","objects/Gui.htm#Minimize",7],
  ["MinParams property (Func)","objects/Func.htm#MinParams",7],
  ["MinSendLevel property (InputHook)","commands/InputHook.htm#MinSendLevel",7],
  ["Mod","commands/Math.htm#Mod",2],
  ["modal (always on top)","commands/MsgBox.htm"],
  ["Modify method (ListView)","commands/ListView.htm#Modify",7],
  ["Modify method (TreeView)","commands/TreeView.htm#Modify",7],
  ["ModifyCol method (ListView)","commands/ListView.htm#ModifyCol",7],
  ["modulo, Mod","commands/Math.htm#Mod"],
  ["Monitor functions","commands/Monitor.htm"],
  ["MonitorGet","commands/MonitorGet.htm",2],
  ["MonitorGetCount","commands/MonitorGetCount.htm",2],
  ["MonitorGetName","commands/MonitorGetName.htm",2],
  ["MonitorGetPrimary","commands/MonitorGetPrimary.htm",2],
  ["MonitorGetWorkArea","commands/MonitorGetWorkArea.htm",2],
  ["MonthCal controls (GUI)","commands/GuiControls.htm#MonthCal"],
  ["mouse hook","commands/InstallMouseHook.htm"],
  ["mouse speed","commands/SetDefaultMouseSpeed.htm"],
  ["mouse wheel","commands/Click.htm"],
  ["MouseClick","commands/MouseClick.htm",2],
  ["MouseClickDrag","commands/MouseClickDrag.htm",2],
  ["MouseGetPos","commands/MouseGetPos.htm",2],
  ["MouseMove","commands/MouseMove.htm",2],
  ["move a window","commands/WinMove.htm"],
  ["move file","commands/FileMove.htm"],
  ["move folder/directory","commands/DirMove.htm"],
  ["Move method (Gui)","objects/Gui.htm#Move",7],
  ["Move method (GuiControl)","objects/GuiControl.htm#Move",7],
  ["MsgBox","commands/MsgBox.htm",2],
  ["mute (changing it)","commands/SoundSetMute.htm"],
  ["Name method/property (Match)","commands/RegExMatch.htm#MatchObject",7],
  ["Name property (Func)","objects/Func.htm#Name",7],
  ["Name property (Gui)","objects/Gui.htm#Name",7],
  ["Name property (GuiControl)","objects/GuiControl.htm#Name",7],
  ["names","Concepts.htm#names"],
  ["nested functions","Functions.htm#nested"],
  ["Nop directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Nop",99],
  ["not","Variables.htm#not",4],
  ["nothing (concepts)", "Concepts.htm#nothing"],
  ["NotifyNonText property (InputHook)","commands/InputHook.htm#NotifyNonText",7],
  ["NoTimers (Thread)","commands/Thread.htm#NoTimers"],
  ["NoTrayIcon","commands/_NoTrayIcon.htm"],
  ["Number","commands/Number.htm",2],
  ["Number","Objects.htm#primitive",6],
  ["number (check if it is one)","commands/Is.htm"],
  ["number format","Concepts.htm#numbers"],
  ["NumGet","commands/NumGet.htm",2],
  ["NumPut","commands/NumPut.htm",2],
  ["Object","objects/Object.htm",6],
  ["object literal","Objects.htm#object-literal"],
  ["Obey directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Obey",99],
  ["Objects (general information)","Objects.htm"],
  ["ObjAddRef","commands/ObjAddRef.htm",2],
  ["ObjBindMethod","commands/ObjBindMethod.htm",2],
  ["ObjFromPtr","Objects.htm#ObjFromPtr",2],
  ["ObjFromPtrAddRef","Objects.htm#ObjFromPtr",2],
  ["ObjGetBase","objects/Any.htm#GetBase",2],
  ["ObjGetCapacity","objects/Object.htm#GetCapacity",2],
  ["ObjHasOwnProp","objects/Object.htm#HasOwnProp",2],
  ["ObjOwnPropCount","objects/Object.htm#OwnPropCount",2],
  ["ObjOwnProps","objects/Object.htm#OwnProps",2],
  ["ObjPtr","Objects.htm#ObjPtr",2],
  ["ObjPtrAddRef","Objects.htm#ObjPtr",2],
  ["ObjRelease","commands/ObjAddRef.htm",2],
  ["ObjSetBase","objects/Object.htm#SetBase",2],
  ["ObjSetCapacity","objects/Object.htm#SetCapacity",2],
  ["OnChar property (InputHook)","commands/InputHook.htm#OnChar",7],
  ["OnClipboardChange","commands/OnClipboardChange.htm",2],
  ["OnCommand (Gui)","objects/GuiOnCommand.htm"],
  ["OnCommand method (GuiControl)","objects/GuiControl.htm#OnCommand",7],
  ["OnEnd property (InputHook)","commands/InputHook.htm#OnEnd",7],
  ["OnError","commands/OnError.htm",2],
  ["OnEvent (Gui)","objects/GuiOnEvent.htm"],
  ["OnEvent method (Gui)","objects/Gui.htm#OnEvent",7],
  ["OnEvent method (GuiControl)","objects/GuiControl.htm#OnEvent",7],
  ["OnExit","commands/OnExit.htm",2],
  ["OnKeyDown property (InputHook)","commands/InputHook.htm#OnKeyDown",7],
  ["OnKeyUp property (InputHook)","commands/InputHook.htm#OnKeyUp",7],
  ["OnMessage","commands/OnMessage.htm",2],
  ["OnNotify (Gui)","objects/GuiOnNotify.htm"],
  ["OnNotify method (GuiControl)","objects/GuiControl.htm#OnNotify",7],
  ["open file","commands/FileOpen.htm"],
  ["operators in expressions","Variables.htm#Operators"],
  ["Opt method (Gui)","objects/Gui.htm#Opt",7],
  ["Opt method (GuiControl)","objects/GuiControl.htm#Opt",7],
  ["or","Variables.htm#or",4],
  ["Ord","commands/Ord.htm",2],
  ["OSError","objects/Error.htm#OSError",6],
  ["OutputDebug","commands/OutputDebug.htm",2],
  ["OwnDialogs (GUI)","objects/Gui.htm#OwnDialogs"],
  ["Owner of a GUI window","objects/Gui.htm#Owner"],
  ["OwnProps method (Object)","objects/Object.htm#OwnProps",7],
  ["parameters passed into a script","Scripts.htm#cmd"],
  ["parse a string (Loop)","commands/LoopParse.htm"],
  ["parse a string (StrSplit)","commands/StrSplit.htm"],
  ["Pause","commands/Pause.htm",2],
  ["performance of scripts","misc/Performance.htm"],
  ["Persistent","commands/Persistent.htm",2],
  ["Picture controls (GUI)","commands/GuiControls.htm#Picture"],
  ["PID (Process ID)","misc/WinTitle.htm#ahk_pid"],
  ["PixelGetColor","commands/PixelGetColor.htm",2],
  ["PixelSearch","commands/PixelSearch.htm",2],
  ["play a sound or video file","commands/SoundPlay.htm"],
  ["Pop method (Array)","objects/Array.htm#Pop",7],
  ["Pos method/property (Match)","commands/RegExMatch.htm#MatchObject",7],
  ["Pos property (File)","objects/File.htm#Pos",7],
  ["PostExec directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#PostExec",99],
  ["PostMessage","commands/PostMessage.htm",2],
  ["power (exponentiation)","Variables.htm#pow"],
  ["prefix and suffix keys","Hotkeys.htm"],
  ["Primitive","Objects.htm#primitive",6],
  ["print a file","commands/Run.htm"],
  ["priority of a process","commands/ProcessSetPriority.htm"],
  ["Priority (Thread)","commands/Thread.htm#Priority"],
  ["Process functions","commands/Process.htm"],
  ["ProcessClose","commands/ProcessClose.htm",2],
  ["ProcessExist","commands/ProcessExist.htm",2],
  ["ProcessGetName","commands/ProcessGetName.htm",2],
  ["ProcessGetPath","commands/ProcessGetName.htm",2],
  ["ProcessGetParent","commands/ProcessGetParent.htm",2],
  ["ProcessSetPriority","commands/ProcessSetPriority.htm",2],
  ["ProcessWait","commands/ProcessWait.htm",2],
  ["ProcessWaitClose","commands/ProcessWaitClose.htm",2],
  ["Progress controls (GUI)","commands/GuiControls.htm#Progress"],
  ["properties (Objects)","Objects.htm#Custom_Classes_property"],
  ["properties of a file or folder","commands/Run.htm"],
  ["PropertyError","objects/Error.htm#MemberError",6],
  ["Prototype property (Class)","objects/Class.htm#Prototype",7],
  ["Ptr property (Buffer)","objects/Buffer.htm#Ptr",7],
  ["Push method (Array)","objects/Array.htm#Push",7],
  ["quit script","commands/ExitApp.htm"],
  ["Radio controls (GUI)","commands/GuiControls.htm#Radio"],
  ["Random","commands/Random.htm",2],
  ["raw-mode Send","commands/Send.htm#SendRaw"],
  ["RawRead method (File)","objects/File.htm#RawRead",7],
  ["RawWrite method (File)","objects/File.htm#RawWrite",7],
  ["read file","commands/FileRead.htm"],
  ["Read method (File)","objects/File.htm#Read",7],
  ["ReadLine method (File)","objects/File.htm#ReadLine",7],
  ["ReadNumType method (File)","objects/File.htm#ReadNum",7],
  ["READONLY","commands/FileGetAttrib.htm"],
  ["reboot","commands/Shutdown.htm"],
  ["Redraw method (GuiControl)","objects/GuiControl.htm#Redraw",7],
  ["reference operator (&)","Variables.htm#ref"],
  ["Reference-Counting","Objects.htm#Refs"],
  ["REG_BINARY","commands/RegRead.htm"],
  ["REG_DWORD","commands/RegRead.htm"],
  ["REG_EXPAND_SZ","commands/RegRead.htm"],
  ["REG_MULTI_SZ","commands/RegRead.htm"],
  ["REG_SZ","commands/RegRead.htm"],
  ["RegCreateKey","commands/RegCreateKey.htm",2],
  ["RegDelete","commands/RegDelete.htm",2],
  ["RegDeleteKey","commands/RegDeleteKey.htm",2],
  ["RegEx: Quick Reference","misc/RegEx-QuickRef.htm"],
  ["RegEx: Callouts","misc/RegExCallout.htm"],
  ["RegEx: SetTitleMatchMode RegEx","commands/SetTitleMatchMode.htm#RegEx"],
  ["RegExMatch","commands/RegExMatch.htm",2],
  ["RegExMatchInfo","commands/RegExMatch.htm#MatchObject",6],
  ["RegExReplace","commands/RegExReplace.htm",2],
  ["registry loop","commands/LoopReg.htm"],
  ["RegRead","commands/RegRead.htm",2],
  ["Regular Expression Callouts","misc/RegExCallout.htm"],
  ["regular expressions: Quick Reference","misc/RegEx-QuickRef.htm"],
  ["regular expressions: RegExMatch","commands/RegExMatch.htm"],
  ["regular expressions: RegExReplace","commands/RegExReplace.htm"],
  ["regular expressions: SetTitleMatchMode RegEx","commands/SetTitleMatchMode.htm#RegEx"],
  ["RegWrite","commands/RegWrite.htm",2],
  ["Reload","commands/Reload.htm",2],
  ["remap joystick","misc/RemapJoystick.htm"],
  ["remap keys or mouse buttons","misc/Remap.htm"],
  ["remote controls, hand-held","scripts/index.htm#WinLIRC"],
  ["remove folder/directory","commands/DirDelete.htm"],
  ["RemoveAt method (Array)","objects/Array.htm#RemoveAt",7],
  ["rename file","commands/FileMove.htm"],
  ["Rename method (Menu)","objects/Menu.htm#Rename",7],
  ["resize a window","commands/WinMove.htm"],
  ["restart the computer","commands/Shutdown.htm"],
  ["Restore method (Gui)","objects/Gui.htm#Restore",7],
  ["Return","commands/Return.htm",3,"E"],
  ["RGB color names","misc/Colors.htm"],
  ["RGB colors","commands/PixelGetColor.htm"],
  ["Round","commands/Math.htm#Round",2],
  ["rounding a number","commands/Math.htm#Round"],
  ["RTrim","commands/Trim.htm",2],
  ["Run","commands/Run.htm",2],
  ["RunAs","commands/RunAs.htm",2],
  ["RunWait","commands/Run.htm",2],
  ["scan code","commands/Send.htm#vk"],
  ["scientific notation","commands/Format.htm"],
  ["Script Showcase","scripts/index.htm"],
  ["Scripts","Scripts.htm"],
  ["Seek method (File)","objects/File.htm#Seek",7],
  ["select file","commands/FileSelect.htm"],
  ["select folder","commands/DirSelect.htm"],
  ["Send","commands/Send.htm",2],
  ["SendEvent","commands/Send.htm#SendEvent",2],
  ["sending data between scripts","commands/OnMessage.htm#ExSendString"],
  ["SendInput","commands/Send.htm#SendInputDetail",2],
  ["SendLevel","commands/SendLevel.htm",2],
  ["SendMessage","commands/SendMessage.htm",2],
  ["SendMode","commands/SendMode.htm",2],
  ["SendPlay","commands/Send.htm#SendPlayDetail",2],
  ["SendText","commands/Send.htm",2],
  ["Set directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#Set",99],
  ["Set method (Map)","objects/Map.htm#Set",7],
  ["SetCapsLockState","commands/SetNumScrollCapsLockState.htm",2],
  ["SetColor method (Menu)","objects/Menu.htm#SetColor",7],
  ["SetControlDelay","commands/SetControlDelay.htm",2],
  ["SetDefaultMouseSpeed","commands/SetDefaultMouseSpeed.htm",2],
  ["SetFont method (Gui)","objects/Gui.htm#SetFont",7],
  ["SetFont method (GuiControl)","objects/GuiControl.htm#SetFont",7],
  ["SetFormat method (GuiControl)","objects/GuiControl.htm#SetFormat",7],
  ["SetIcon method (Menu)","objects/Menu.htm#SetIcon",7],
  ["SetImageList method (ListView)","commands/ListView.htm#SetImageList",7],
  ["SetImageList method (TreeView)","commands/TreeView.htm#SetImageList",7],
  ["SetKeyDelay","commands/SetKeyDelay.htm",2],
  ["SetMainIcon directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#SetMainIcon",99],
  ["SetMouseDelay","commands/SetMouseDelay.htm",2],
  ["SetNumLockState","commands/SetNumScrollCapsLockState.htm",2],
  ["SetProp directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#SetProp",99],
  ["SetRegView","commands/SetRegView.htm",2],
  ["SetScrollLockState","commands/SetNumScrollCapsLockState.htm",2],
  ["SetStoreCapsLockMode","commands/SetStoreCapsLockMode.htm",2],
  ["SetTimer","commands/SetTimer.htm",2],
  ["SetTitleMatchMode","commands/SetTitleMatchMode.htm",2],
  ["SetWinDelay","commands/SetWinDelay.htm",2],
  ["SetWorkingDir","commands/SetWorkingDir.htm",2],
  ["short file name (8.3 format)","commands/LoopFiles.htm#LoopFileShortPath"],
  ["short-circuit boolean evaluation","Functions.htm#ShortCircuit"],
  ["shortcut file","commands/FileCreateShortcut.htm"],
  ["Show method (Gui)","objects/Gui.htm#Show",7],
  ["Show method (Menu)","objects/Menu.htm#Show",7],
  ["Shutdown","commands/Shutdown.htm",2],
  ["Silent Install/Uninstall","Program.htm#install"],
  ["Sin","commands/Math.htm#Sin",2],
  ["SingleInstance","commands/_SingleInstance.htm"],
  ["Size (Gui event)","objects/GuiOnEvent.htm#Size"],
  ["size of a file/folder","commands/FileGetSize.htm"],
  ["size of a window","commands/WinGetPos.htm"],
  ["Size property (Buffer)","objects/Buffer.htm#Size",7],
  ["Sleep","commands/Sleep.htm",2],
  ["Slider controls (GUI)","commands/GuiControls.htm#Slider"],
  ["Sort","commands/Sort.htm",2],
  ["Sound functions","commands/Sound.htm"],
  ["SoundBeep","commands/SoundBeep.htm",2],
  ["SoundGetInterface","commands/SoundGetInterface.htm",2],
  ["SoundGetMute","commands/SoundGetMute.htm",2],
  ["SoundGetName","commands/SoundGetName.htm",2],
  ["SoundGetVolume","commands/SoundGetVolume.htm",2],
  ["SoundPlay","commands/SoundPlay.htm",2],
  ["SoundSetMute","commands/SoundSetMute.htm",2],
  ["SoundSetVolume","commands/SoundSetVolume.htm",2],
  ["space","commands/Is.htm"],
  ["spinner control (GUI)","commands/GuiControls.htm#UpDown"],
  ["SplitPath","commands/SplitPath.htm",2],
  ["splitting long lines","Scripts.htm#continuation"],
  ["Sqrt","commands/Math.htm#Sqrt",2],
  ["Stack property (Error)","objects/Error.htm#Stack",7],
  ["standard library","Scripts.htm#lib"],
  ["standard output (stdout)","commands/FileAppend.htm"],
  ["Start method (InputHook)","commands/InputHook.htm#Start",7],
  ["static","Functions.htm#static",5],
  ["static functions","Functions.htm#static-functions"],
  ["static variables","Functions.htm#static"],
  ["StatusBar controls (GUI)","commands/GuiControls.htm#StatusBar"],
  ["StatusBar.SetIcon","commands/GuiControls.htm#SB_SetIcon"],
  ["StatusBar.SetParts","commands/GuiControls.htm#SB_SetParts"],
  ["StatusBar.SetText","commands/GuiControls.htm#SB_SetText"],
  ["StatusBarGetText","commands/StatusBarGetText.htm",2],
  ["StatusBarWait","commands/StatusBarWait.htm",2],
  ["Stop method (InputHook)","commands/InputHook.htm#Stop",7],
  ["StrCompare","commands/StrCompare.htm",2],
  ["StrGet","commands/StrGet.htm",2],
  ["String","commands/String.htm",2],
  ["String","Objects.htm#primitive",6],
  ["string (search for)","commands/InStr.htm"],
  ["string: InStr","commands/InStr.htm"],
  ["string: SubStr","commands/SubStr.htm"],
  ["strings (concepts)","Concepts.htm#strings"],
  ["StrLen","commands/StrLen.htm",2],
  ["StrLower","commands/StrLower.htm",2],
  ["StrPtr","commands/StrPtr.htm",2],
  ["StrPut","commands/StrPut.htm",2],
  ["StrReplace","commands/StrReplace.htm",2],
  ["StrSplit","commands/StrSplit.htm",2],
  ["StrTitle","commands/StrLower.htm",2],
  ["StrUpper","commands/StrLower.htm",2],
  ["structures, via DllCall","commands/DllCall.htm#struct"],
  ["styles for GUI methods","misc/Styles.htm"],
  ["Submit method (Gui)","objects/Gui.htm#Submit",7],
  ["SubStr","commands/SubStr.htm",2],
  ["super","Objects.htm#Custom_Classes_super"],
  ["Suspend","commands/Suspend.htm",2],
  ["suspend or hibernate","commands/Shutdown.htm#ExSuspend"],
  ["Switch","commands/Switch.htm",3,"E"],
  ["SysGet","commands/SysGet.htm",2],
  ["SysGetIPAddresses","commands/SysGetIPAddresses.htm",2],
  ["Tab controls (GUI)","commands/GuiControls.htm#Tab"],
  ["Tan","commands/Math.htm#Tan",2],
  ["TargetError","objects/Error.htm#TargetError",6],
  ["terminate a window","commands/WinKill.htm"],
  ["terminate script","commands/ExitApp.htm"],
  ["ternary operator (?:)","Variables.htm#ternary"],
  ["Text controls (GUI)","commands/GuiControls.htm#Text"],
  ["text-mode Send","commands/Send.htm#SendText"],
  ["Text property (GuiControl)","objects/GuiControl.htm#Text",7],
  ["ThisHotkey","Hotkeys.htm#ThisHotkey"],
  ["Thread","commands/Thread.htm",2],
  ["threads","misc/Threads.htm"],
  ["Throw","commands/Throw.htm",3,"E"],
  ["time","commands/Is.htm"],
  ["Timeout property (InputHook)","commands/InputHook.htm#Timeout",7],
  ["TimeoutError","objects/Error.htm#TimeoutError",6],
  ["Timer (timed subroutines)","commands/SetTimer.htm"],
  ["times and dates (compare)","commands/DateDiff.htm"],
  ["times and dates (math)","commands/DateAdd.htm"],
  ["times and dates (of files)","commands/FileSetTime.htm"],
  ["title of a window","commands/WinSetTitle.htm"],
  ["Title property (Gui)","objects/Gui.htm#Title",7],
  ["ToggleCheck method (Menu)","objects/Menu.htm#ToggleCheck",7],
  ["ToggleEnable method (Menu)","objects/Menu.htm#ToggleEnable",7],
  ["ToolTip","commands/ToolTip.htm",2],
  ["transparency of a window","commands/WinSetTransparent.htm"],
  ["tray icon","Program.htm#tray-icon"],
  ["tray menu (customizing)","Variables.htm#TrayMenu"],
  ["TraySetIcon","commands/TraySetIcon.htm",2],
  ["TrayTip","commands/TrayTip.htm",2],
  ["TreeView controls (GUI)","commands/TreeView.htm"],
  ["Trim","commands/Trim.htm",2],
  ["True","Variables.htm#Boolean",1],
  ["Try","commands/Try.htm",3,""],
  ["Tutorial","Tutorial.htm"],
  ["Type","commands/Type.htm",2],
  ["Type property (GuiControl)","objects/GuiControl.htm#Type",7],
  ["TypeError","objects/Error.htm#TypeError",6],
  ["Uncheck method (Menu)","objects/Menu.htm#Uncheck",7],
  ["Unset","Language.htm#unset",1],
  ["UnsetError","objects/Error.htm#UnsetError",6],
  ["UnsetItemError","objects/Error.htm#UnsetError",6],
  ["uninitialised variables","Concepts.htm#uninitialised-variables"],
  ["Until","commands/Until.htm",3,"E"],
  ["UpdateManifest directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#UpdateManifest",99],
  ["UpDown controls (GUI)","commands/GuiControls.htm#UpDown"],
  ["UseHook","commands/_UseHook.htm"],
  ["user (run as a different user)","commands/RunAs.htm"],
  ["user library","Scripts.htm#lib"],
  ["UseResourceLang directive (Ahk2Exe)","misc/Ahk2ExeDirectives.htm#UseResourceLang",99],
  ["UseTab method (GuiControl)","objects/GuiControl.htm#UseTab",7],
  ["Value property (GuiControl)","objects/GuiControl.htm#Value",7],
  ["ValueError","objects/Error.htm#ValueError",6],
  ["variables","Variables.htm"],
  ["variables, built-in","Variables.htm#BuiltIn"],
  ["variables, ListVars","commands/ListVars.htm"],
  ["variables, type of data","commands/Is.htm"],
  ["variadic functions","Functions.htm#Variadic"],
  ["variants (duplicate hotkeys and hotstrings)","commands/_HotIf.htm#variant"],
  ["VarRef","Concepts.htm#variable-references",6],
  ["VarSetStrCapacity","commands/VarSetStrCapacity.htm",2],
  ["VerCompare","commands/VerCompare.htm",2],
  ["version of a file","commands/FileGetVersion.htm"],
  ["virtual key","commands/Send.htm#vk"],
  ["Visible property (GuiControl)","objects/GuiControl.htm#Visible",7],
  ["VisibleNonText property (InputHook)","commands/InputHook.htm#VisibleNonText",7],
  ["VisibleText property (InputHook)","commands/InputHook.htm#VisibleText",7],
  ["volume (changing it)","commands/SoundSetVolume.htm"],
  ["wait (sleep)","commands/Sleep.htm"],
  ["wait for a key to be released or pressed","commands/KeyWait.htm"],
  ["Wait method (InputHook)","commands/InputHook.htm#Wait",7],
  ["What property (Error)","objects/Error.htm#What",7],
  ["Wheel hotkeys for mouse","Hotkeys.htm#Wheel"],
  ["Wheel, simulating rotation","commands/Click.htm"],
  ["While","commands/While.htm",3,"E"],
  ["wildcards (for files & folders)","commands/LoopFiles.htm"],
  ["Win functions","commands/Win.htm"],
  ["WinActivate","commands/WinActivate.htm",2],
  ["WinActivateBottom","commands/WinActivateBottom.htm",2],
  ["WinActivateForce","commands/_WinActivateForce.htm"],
  ["WinActive","commands/WinActive.htm",2],
  ["Winamp automation","misc/Winamp.htm"],
  ["WinClose","commands/WinClose.htm",2],
  ["window group","misc/WinTitle.htm#ahk_group"],
  ["window messages","misc/SendMessageList.htm"],
  ["WinExist","commands/WinExist.htm",2],
  ["WinGetClass","commands/WinGetClass.htm",2],
  ["WinGetClientPos","commands/WinGetClientPos.htm",2],
  ["WinGetControls","commands/WinGetControls.htm",2],
  ["WinGetControlsHwnd","commands/WinGetControlsHwnd.htm",2],
  ["WinGetCount","commands/WinGetCount.htm",2],
  ["WinGetExStyle","commands/WinGetStyle.htm",2],
  ["WinGetID","commands/WinGetID.htm",2],
  ["WinGetIDLast","commands/WinGetIDLast.htm",2],
  ["WinGetList","commands/WinGetList.htm",2],
  ["WinGetMinMax","commands/WinGetMinMax.htm",2],
  ["WinGetPID","commands/WinGetPID.htm",2],
  ["WinGetPos","commands/WinGetPos.htm",2],
  ["WinGetProcessName","commands/WinGetProcessName.htm",2],
  ["WinGetProcessPath","commands/WinGetProcessPath.htm",2],
  ["WinGetStyle","commands/WinGetStyle.htm",2],
  ["WinGetText","commands/WinGetText.htm",2],
  ["WinGetTitle","commands/WinGetTitle.htm",2],
  ["WinGetTransColor","commands/WinGetTransColor.htm",2],
  ["WinGetTransparent","commands/WinGetTransparent.htm",2],
  ["WinHide","commands/WinHide.htm",2],
  ["WinKill","commands/WinKill.htm",2],
  ["WinLIRC, connecting to","scripts/index.htm#WinLIRC"],
  ["WinMaximize","commands/WinMaximize.htm",2],
  ["WinMinimize","commands/WinMinimize.htm",2],
  ["WinMinimizeAll","commands/WinMinimizeAll.htm",2],
  ["WinMinimizeAllUndo","commands/WinMinimizeAll.htm",2],
  ["WinMove","commands/WinMove.htm",2],
  ["WinMoveBottom","commands/WinMoveBottom.htm",2],
  ["WinMoveTop","commands/WinMoveTop.htm",2],
  ["WinRedraw","commands/WinRedraw.htm",2],
  ["WinRestore","commands/WinRestore.htm",2],
  ["WinSetAlwaysOnTop","commands/WinSetAlwaysOnTop.htm",2],
  ["WinSetEnabled","commands/WinSetEnabled.htm",2],
  ["WinSetExStyle","commands/WinSetStyle.htm",2],
  ["WinSetRegion","commands/WinSetRegion.htm",2],
  ["WinSetStyle","commands/WinSetStyle.htm",2],
  ["WinSetTitle","commands/WinSetTitle.htm",2],
  ["WinSetTransColor","commands/WinSetTransColor.htm",2],
  ["WinSetTransparent","commands/WinSetTransparent.htm",2],
  ["WinShow","commands/WinShow.htm",2],
  ["WinSize (via WinMove)","commands/WinMove.htm"],
  ["WinTitle","misc/WinTitle.htm"],
  ["WinWait","commands/WinWait.htm",2],
  ["WinWaitActive","commands/WinWaitActive.htm",2],
  ["WinWaitClose","commands/WinWaitClose.htm",2],
  ["WinWaitNotActive","commands/WinWaitActive.htm",2],
  ["WM_* (Windows messages)","misc/SendMessageList.htm"],
  ["WM_COPYDATA","commands/OnMessage.htm#ExSendString"],
  ["working directory","commands/SetWorkingDir.htm"],
  ["wParam","commands/SendMessage.htm"],
  ["write file","commands/FileAppend.htm"],
  ["Write method (File)","objects/File.htm#Write",7],
  ["WriteLine method (File)","objects/File.htm#WriteLine",7],
  ["WriteNumType method (File)","objects/File.htm#WriteNum",7],
  ["WS_* (GUI styles)","misc/Styles.htm"],
  ["XButton","KeyList.htm#mouse-advanced"],
  ["YYYYMMDDHH24MISS","commands/FileSetTime.htm#YYYYMMDD"],
  ["ZeroDivisionError","objects/Error.htm#ZeroDivisionError",6],
  ["{Blind}","commands/Send.htm#blind"]
];