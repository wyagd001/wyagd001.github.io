...
#### <a id="hotkey">快捷键</a>
- Mbutton 默认的快捷键是鼠标中键, 可以右键托盘菜单的热键管理菜单自定义为其他按键. 按下后显示一个动作主面板, 主面板由上面的 12 个按钮(针对当前窗口)和下面的 12 个按钮(针对选中的文字, 或文件, 由 Ctrl+C 获取) 组成, 其中每个按钮执行一个指定的动作.  
- 鼠标手势 默认的鼠标手势"上右"来显示主面板, 可以在配置文件 "如一.ini" 中修改为其他.  
  - 下图是主面板图片  
...
#### <a  id="Action">动作</a>  
  - 动作的的设置保存在 "配置文件/动作列表.ini" 文件中. 动作有以下 9 种类别: 运行(Run), 面板(Gui), 菜单(Menu), 发送按键(Key), 内置命令和函数(Cando 和 CanFunc), 使用指定程序打开选中文件(OpenWith), 设置剪贴板(SetClip), 对话框(Msgbox). 面板上的按钮对应一个动作, 点击面板上的按钮就会执行相应的动作.  
     ...
     - .6 使用指定程序打开选中文件: openwith&#124;Notepad.exe 使用记事本打开选中的文件  
     - .7 设置剪贴板: 将剪贴板设为某个变量的值. 例: setclip&#124;%CandySel% 将选中文本或文件(的路径) 放入剪贴板  
     - .8 对话框: 测试用, 显示变量的值.  
  ...
  -  版本[1.5]  
     -  .1 新增 动作可以设置单独的热键, 如 "^!a", 表示 Ctrl + Alt + a 热键  
     -  .2 新增 动作可以设置单独的鼠标手势(上下左右), 如 "左", 表示按下鼠标右键后向左拖, 默认配置是将窗口放到左半屏幕.  
     -  .3 默认配置中 鼠标手势 "上右"  打开主界面, 可以在配置文件 "如一.ini" 中修改, 也可以整体关闭鼠标手势.  
