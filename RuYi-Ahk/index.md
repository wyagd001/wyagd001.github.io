...
- **<span id="Action">动作</span>**  
  ...
  版本[1.5]  
  .1 新增 动作的单独热键, 如 "^!a", 表示 Ctrl + Alt + a 热键  
  .2 新增 动作的单独鼠标手势, 如 "左", 表示按下鼠标右键想左拖, 默认配置是将窗口放到左半屏幕.  
  .3 默认配置中  鼠标手势 "上右"  打开主界面, 可以在配置文件 "如一.ini" 中修改, 也可以整体关闭鼠标手势.  
  ![Image text](../img/动作管理.jpg)

- **<span id="Script">外部脚本</span>**
  - 许多动作是由外部的 ahk 脚本来完成的, 它们统一放在 "外部脚本" 目录. 现在内置的外部动作脚本有 84 个.  
...
|1222|否|360收藏夹|无条件|将360浏览器的收藏夹显示为菜单
|1223|是|Chrome收藏夹|无条件|将Chrome浏览器的收藏夹显示为菜单
|1224|是|Edge收藏夹|无条件|将Edge浏览器的收藏夹显示为菜单
