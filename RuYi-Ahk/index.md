...
每个动作代表一个预设好的命令, 面板上的按钮对应一个动作, 点击面板上的按钮就会执行相应的动作. 动作设置保存在[\[1.1+\]](./history.md#1.1) "配置文件/内置动作.ini" 和 "配置文件/自定义动作.ini" 文件中.  
  ...
  - 动作有以下 9 种类别: 运行(Run), 面板(Gui), 菜单(Menu), 发送按键(Key), 内置命令和函数(Cando 和 CanFunc), 使用指定程序打开选中文件(OpenWith), 设置剪贴板(SetClip), 对话框(Msgbox), 动作组合(CombActions).  
  ...
  - 动作的管理由托盘菜单的 "[动作管理]((./Actions/1096.md))" 和面板按钮编辑界面的 "转到动作编辑" 按钮来启动.   
