...
**当前版本**: [1.7 - 2023/04/08](history.md)
...
#### <a  id="Action">动作</a>  
  - 动作的的设置保存在 "配置文件/动作列表.ini" 文件中. 动作有以下类别: 运行(Run), 面板(Gui), 菜单(Menu), 发送按键(Key), 内置命令和函数(Cando 和 CanFunc) 等. 面板上的按钮对应一个动作, 点击面板上的按钮就会执行相应的动作.  
     - 运行: 运行程序, 打开文件(文件夹, 网址). 如打开C盘写为 "Run&#124;C:", 打开记事本写为 "Run&#124;notepad.exe". 要运行的外部 Ahk 脚本统一放在 "外部脚本" 目录.  
  ...
  -  版本[1.5]  
     ...
     -  .2 新增 动作可以设置单独的鼠标手势, 如 "左", 表示按下鼠标右键向左拖, 默认配置是将窗口放到左半屏幕.  
     -  .3 默认配置中 鼠标手势 "上右"  打开主界面, 可以在配置文件 "如一.ini" 中修改, 也可以整体关闭鼠标手势.  

#### <a id="Script">外部脚本</a>
  - 许多动作是由外部的 ahk 脚本来完成的, 它们统一放在 "外部脚本" 目录. 现在内置的外部动作脚本有 92 个.  
