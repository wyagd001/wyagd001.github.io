...
- 额外的热键: 鼠标手势(仅上下左右四个方向)[1.5+], 使用鼠标手势 "上右" 来显示主面板, 也可以在配置文件 "如一.ini" 中修改为其他, 或整体关闭鼠标手势.  
...
每个动作代表一个预设好的命令, 面板上的按钮对应一个动作, 点击面板上的按钮就会执行相应的动作. 其设置保存在 "配置文件/动作列表.ini" 文件中.  
  ...
  - 动作有以下 9 种类别: 运行(Run), 面板(Gui), 菜单(Menu), 发送按键(Key), 内置命令和函数(Cando 和 CanFunc), 使用指定程序打开选中文件(OpenWith), 设置剪贴板(SetClip), 对话框(Msgbox), 动作组合(CombActions)[2.4+].  
  ...
  -  动作除了通过通过面板或菜单运行外, 还可以通过以下方式运行  
     ...
     -  .3 [\[1.8+\]](./history.md#1.8)动作可以通过热字串来启动打开. 例: 默认的 "jsb" 来打开记事本  
     -  .4 [\[1.7+\]](./history.md#1.7)通过动作搜索界面来打开  
     -  .5 [\[1.7+\]](./history.md#1.7)设置动作悬浮为按钮, 可以通过点击单个动作的悬浮按钮来打开.  
     -  .6 [\[1.9+\]](./history.md#1.9)在如意运行时, 通过运行主程序加动作编号的方式, 如 "如一.exe 1017" 来打开指定动作  
     -  .7 [\[1.9+\]](./history.md#1.9)如意固定到任务栏, 通过任务栏按钮菜单点击菜单项目打开最近的某个动作(可固定动作)(方式同 6)  
     -  .8 [\[2.0+\]](./history.md#2.0)设置跟随或定时动作, 启动如意时自动打开或定时打开  
     -  .9 [\[2.1+\]](./history.md#2.1)设置文件右键菜单,  通过资源管理器中文件的右键菜单打开(方式同 6)  
     -  .10 [\[2.3+\]](./history.md#2.3)设置动作到托盘按钮, 通过点击托盘按钮来打开  
     -  .11 [\[2.1+\]](./history.md#2.1)通过网页来打开执行动作(任意窗口和无条件的动作)  
