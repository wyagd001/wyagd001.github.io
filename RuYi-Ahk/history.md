...
# [](#header-2) 更新历史
### [](#header-3) <p id="2.4">2.4 - 2023/09/XX</p>  
1. 新增 动作管理界面, 如果动作是外部脚本, 并且 ATA 内置, 可设置默认运行外部脚本, 而不是默认执行 ATA 内置动作  
2. 新增 动作管理界面, 如果动作存在在线帮助文档, 添加一个打开网页的按钮  
3. 新增 如意动作类型 CombActions, 动作组合, 如 CombActions\|1519-1168, 依次执行动作1519, 1168  
3. 新增 外部脚本  
.01. 图像文件_指定颜色变透明.ahk  
.02. 图像文件_颜色替换.ahk  
.03. 图像文件_转为Ico文件.ahk  
...
.10.  
4. 修改 外部脚本  
.01. 图像缩小一半.ahk 支持格式转换和多文件(如, jpg 或 png)  
.02. 图像质量压缩一半.ahk 支持多文件  
.03. 图像变灰度.ahk 支持多文件  
.04. 文本文件中查找字符.ahk 文件类型的编辑框改为组合框  
.05. 通过桌面权限打开文件(ShellRun).ahk 支持带有一个参数  
.06. 通过当前用户的权限新建任务计划并执行打开文件.ahk 支持带有参数  
.07. 任务栏按钮窗口热键切换.ahk 适配屏幕大小(但任务栏要在底部)  
.08. 查找重复文件.ahk 支持多个文件夹, 以 | 或 ; 分隔  
.09.  
5. 修复 Bug  

### [](#header-3) <p id="2.3">2.3 - 2023/09/14</p>  
...
.01 同时编辑两个同名的文件.ahk  
.02 输入法指示器.ahk  
.03 当前用户回收站中的文件信息.ahk  
.04 取色小程序.ahk  
.05 Ahk帮助打开的页面.ahk  
.06 暂时隐藏窗口.ahk  
.07 文件_新建.ahk  一键新建文本文件或文件夹   
.08 记事本打开浏览器当前浏览的本地文件.ahk  
.09 隐藏窗口并静音.ahk  
.10 保存和恢复桌面图标位置.ahk  
.11 托盘图标管理器.ahk  
.12 通过当前用户的权限新建任务计划并执行打开文件.ahk  
.13 通过桌面权限打开文件(ShellRun).ahk  
...
.19 动态缩略图.ahk  
17. 修改 外部脚本动作  
.1 数字_查看代码对应的颜色  能手动输入颜色代码来查看对应颜色  
.2 文本处理 Base64 解密能指定按 UTF-16 来读取解密出来的文字(以前默认为 UTF-8)  
.3 文本批量操作  支持多行文本的查找和替换  
.4 PathU 函数由 v0.90 更新到 v0.91  
.5 文件夹同步 如果目标文件夹存在 .忽略列表.ini 文件, 则使用目标文件夹的配置文件(以满足一对多的不同需求)  

### [](#header-3) <p id="2.2">2.2 - 2023/08/17</p>  
1. 新增 如意的设置界面  
...
.01 屏蔽键盘.ahk  
.02 移动鼠标.ahk  
.03 文件_额外的属性(ExtendedProperty).ahk  
.04 文件_额外的属性(GetDetailsOf).ahk  
.05 磁盘_挂载为文件夹.ahk  
.06 文件夹_取消挂载.ahk  
.07 文件夹_虚拟磁盘.ahk  
.08 文件夹_卸载虚拟磁盘.ahk  
...
16. 修改 外部脚本  
.1 文本的 打开选中.ahk ① 需按住 shift 才保存写入 Ini_Fav 的 Ini 文件 ② 新增对 clsid 的支持   
.2 对话框_收藏夹.ahk 适应新的 ini 样式  
.3 桌面图标设置.ahk  针对不同的电脑增强适配性  
.4 Ini_Fav.ahk 修改不能执行如意动作的 Bug  
.5 文本处理.ahk 功能增强  
.6 文件预览.ahk 功能增强  

### [](#header-3) <p id="2.1">2.1 - 2023/07/29</p>  
...
.02 批量重命名.ahk  
.03 文本对比(WebView2).ahk2  需要安装 WebView2  
.04 俄罗斯方块.ahk2  
.05 播放器控制.ahk  
.06 画十字.ahk  
.07 鼠标尾巴.ahk  
...
.22 系统信息.ahk  
7. 修改 外部脚本  
.1 截图.ahk 参数支持自定义保存路径  
.2 文本文件批量操作.ahk 替换支持 \r 转义  
.3 Ini_收藏夹菜单.ahk 修错  
.4 文件_设置壁纸.ahk 参数支持第 2 块屏幕  
8. 修复一些 Bug  
9. 更新 ahk 最新的帮助文件  

### [](#header-3) <p id="2.0">2.0 - 2023/07/05</p>  
...
.14 Base64解密为图片.ahk  
.15 图像转为Base64.ahk  
.16 剪贴板中的Base64转html后粘贴.ahk  
.17 剪贴板中的Base64转图片后粘贴.ahk  
6. 修改 外部脚本  
.1 改善中英文符号转换  

### [](#header-3) <p id="1.9">1.9 - 2023/06/07</p>  
...
.02 文件夹_文件列表(整合版).ahk  
.03 文件夹_文件列表.ahk  
.04 文件夹_文件列表(按创建时间降序).ahk  
.05 文件夹_文件列表(按最近修改时间降序).ahk  
.06 文件夹_文件列表(按文件大小降序).ahk  
.07 复制资源管理器各列的信息.ahk  
.08 管理文件ADS.ahk  
.09 添加文本到 ADS 流.ahk  
.10 剪贴板中源代码转纯文本后粘贴.ahk  
.11 剪贴板中源代码转html后粘贴.ahk  
...
.22 管理文本文件编码.ahk  
6. 修改的 外部脚本  
.1 文本文件中查找字符.ahk 修复无法正确查找 utf-8-raw 编码文件内容的问题  
.2 文本文件转码.ahk Unicode BE 编码转为其他编码时长度不对的问题  
.3 查找重复文件.ahk 查找重复文件能指定扩展名查找  
.4 Chrome 收藏夹菜单 支持两级文件夹  
.5 文本文件中查找字符.ahk 搜索文件内容时指定的扩展名先进行编码判定  
7. 修复一些 Bug  

### [](#header-3) <p id="1.8">1.8 - 2023/04/27</p>  
1. 新增 热字串相关功能  
...
5. 新增 外部脚本  
.1 Adb.ahk  
.2 文件临时共享.ahk  局域网 http 共享文件  
.3 查找重复文件.ahk  查找选中文件夹内的重复文件  
.4 窗口微缩.ahk      窗口以缩略图形式置顶显示  
.5 7zip压缩.ahk  
.6 7zip智能解压.ahk  
.7 UnRar.ahk  
6. 修改的 外部脚本  
.1 文本处理 新增一些功能项目  
.2 同步文件夹 可以单独同步当前选中的单个文件
7. 修复一些 Bug  

### [](#header-3) <p id="1.7">1.7 - 2023/04/08</p>  
...
.2 对话框_收藏夹.ahk  
.3 文本处理.ahk 对选中文本进行处理  
.4 复制文件夹结构到当前目录.ahk  
7. 修改 外部脚本  
.1 对话框.ahk 重命名为 对话框_打开目录.ahk 能显示 ini 中收藏的条目  
.2 数据库_文件库中搜索文件.ahk 数据库能单个项目计算MD5并存储进db文件  
.3 文本文件转码.ahk 新增文件名不变并删除源文件的选项  
.4 浏览器收藏夹菜单能支持文件夹  
8. 一些小的修改  

### [](#header-3) <p id="1.6">1.6 - 2023/03/09</p>  
1. 按键模式文字对齐  
2. 面板管理新增重命名和复制功能  
...
.7 文本文件内容放入剪贴板.ahk  
.8 对话框.ahk 显示一个当前打开的文件夹的右键菜单, 对话框跳转到选择的项目  
.9 搜狗翻译.ahk  
.10 Environment.ahk  将打开的文件夹路径加入 Path 变量中  
.11 环境变量.ahk  环境变量编辑备份脚本  
6. 一些小的修改  

### [](#header-3) <p id="1.5">1.5 - 2023/02/21</p>  
1. 新增 指定单独的热键来打开动作  
2. 新增 指定鼠标手势来打开动作  
...
### [](#header-3) <p id="1.4">1.4 - 2023/02/04</p>  
1. 新增 记录动作运行的次数和时间  
...
### [](#header-3) <p id="1.3">1.3 - 2023/01/24</p>  
1. 程序改为双进程模式(由 如一.exe + AnyToAhk.exe 组成)  
.1 如一.exe 负责热键调用, 界面展示, 动作管理.  
.2 AnyToAhk.exe 为大多数外部脚本的集合版本, 运行外部脚本动作时, 大部分脚本不再启动新的 Ahk 进程.  
2. 新增 文件夹同步脚本. 左侧文件夹同步到右侧文件夹.  

### [](#header-3) <p id="1.1">1.1 - 2022/12/22</p>  
1. 新增菜单模式  
2. 完善动作和面板的管理  
