...
### <span id="2.5">2.5 - 2024/02/XX</span>
1. 测试 当修改保存内置动作时, 使用 内置动作自定义.ini 文件保存修改内容(即更新主程序时不会覆盖修改的内置动作配置)  
2. 新增 内部标签动作  
.01 将选中文件添加为自定义动作  
.02 将选中文件的快捷方式发送到 Lnk 文件夹  
.03 将选中文本发送到 #32770 对话框并发送指定按键 函数调用原型 SendTo32770(CandySel, Key) 例如动作 1547: canfunc|SendTo32770|%CandySel%|{Enter}  
3. 新增 外部脚本动作  
...
.09 显示剪贴板内容.ahk  
.10 多文件合并.ahk(未完成)  
.11 激活指定输入法.ahk(暂时只预设了搜狗拼音)  
4. 修改 外部脚本动作  
.01 [查找重复文件.ahk](/Actions/1229.md)  文件列表按 Md5 值排序, 新增 1 列以便能清晰显示分组  
.02 [文件夹_文件列表(整合版).ahk](/Actions/1297.md) 增加更多选项  
.03 区域截图.ahk  支持输入文件名后保存  
.04 剪贴板保存为文件到当前文件夹.ahk  未指定文件目录时, 默认保存到桌面  
.05 当前用户回收站中的文件信息.ahk 修改 SID 的获取方式  
.06 文本文件中查找字符.ahk 编辑文件按钮支持跳转到关键字所在行  
.07 管理文本文件编码.ahk  修复没有指定正确的输出编码的问题  
.08 文本处理.ahk  新增获取单个多音字的拼音的功能  
.09  
5. 修复 Bug
