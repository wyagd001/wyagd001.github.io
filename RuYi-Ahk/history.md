...
### <span id="2.6">2.7 - 2024/06/16</span>
1. 新增 如意内置函数 InvokeVerb 执行资源管理器右键菜单的直接执行(Shell动词 如复制, 剪切, 删除, 属性等)
2. 新增 动作管理界面 列表项目右键菜单 发送无条件的动作快捷方式到桌面
3. 新增 动作管理界面 动作热键支持单键的双击和长按参数(如 `|||长按 或 `|||双击)
4. 新增 ATA 内置动作  
.01 额外任务栏文本区 显示网速  
.02 额外任务栏文本区 CPU占用  
5. 新增 外部脚本动作  
